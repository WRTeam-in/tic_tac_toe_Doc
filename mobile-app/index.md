---
sidebar_position: 1
---

# Mobile Application

Thank you for purchasing **Tic Tac Toe**. We appreciate your support. In this documentation, you will find instructions on how to set up the mobile application for Tic Tac Toe.

import DocBanner from '@site/src/components/DocBanner/DocBanner';

<DocBanner />

## Contact Information

**By:** WRTeam
**Email:** [support@wrteam.in](mailto:support@wrteam.in)

## About This Documentation

This documentation provides comprehensive instructions for setting up and customizing the Tic Tac Toe mobile application. You'll find detailed guides for:

- Setting up the Flutter development environment (see the [Flutter Setup Guide](https://www.marketplace.wrteam.in/docs/flutter-common-doc/GeneralSettings/basicsetup/))
- Customizing app appearance and branding
- Integrating with Firebase (Authentication, Realtime Database, Storage)
- Configuring ads, monetization, and in-app purchases
- Configuring language and localization
- Generating release versions and publishing to the stores

For a look at what the product itself offers, see the [Features documentation](/features/).

If you have any questions, feel free to reach out. Thank you for choosing Tic Tac Toe!
